...
_BINANCE_INTERVAL = {"1D": "1d", "4H": "4h", "1H": "1h"}
...
def render_live_scanner(ticker, timeframe, min_body_pct, min_vol_mult,
    ...
    for i, (idx, row) in enumerate(scan.iterrows()):
        ...
        table_rows.append({
            "#":        len(scan) - i,
            "Date":     (
                (idx + pd.Timedelta(hours=7)).strftime("%Y-%m-%d %H:%M WIB")
    ...
    with tab3:
        ...
        if not best_trades:
            ...
            def _fmt_wib(ts):
                ...
                if timeframe in ("4H", "1H"):
                    return ts.strftime("%Y-%m-%d %H:%M WIB")
                return ts.strftime("%Y-%m-%d")

            for c in ["Trigger Date","Entry Date","Exit Date"]:
                log[c] = pd.to_datetime(log[c]).apply(_fmt_wib)
