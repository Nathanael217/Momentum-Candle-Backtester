...
def _scanner_ai_verdict(sig: dict, ml: dict = None, bt: dict = None, wfo: dict = None) -> dict:
    ...
    if ml:
        ...
        if _ml_trained_p:
            _cv_str_p = f"CV Accuracy: {_ml_cv_p*100:.1f}%" if _ml_cv_p is not None else "CV: n/a"
            ...
            ml_section = (
                f"ML Probability: {ml['pct']:.1f}% ({ml['label']}) | "
                f"Model: {_ml_mname_p} trained on n={_ml_ns_p} samples labeled by {_cfg_str_p} | "
                f"{_cv_str_p}"
                + (f" | Top features: {_top3}" if _top3 else "")
            )
        else:
            ml_section = (
                f"ML Probability: {ml['pct']:.1f}% ({ml['label']}) — "
                f"heuristic only ({_ml_mname_p}), not trained on outcomes"
            )
    else:
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:12px;">
    ...
    <div style="margin-top:6px;">
      ...
      <div style="color:#ccd6f6;">Price already >1R from aggressive entry without a retrace</div>
            ...
            # ── Step 2: Train ML (two candidates side-by-side) ───────────────
            if _bt_ready:
                ...
                def _cand_stats_line(c, kind):
                    if not c:
                        return "no valid method found"
                    if kind == "A":
                        nb = c.get("newest_bucket", {}) or {}
                        return (f"Newest bucket — WR {nb.get('wr',0):.1f}% · "
                                f"EV {nb.get('ev',0):+.2f}R · n={nb.get('n',0)}")
                    else:
                        return (f"Weighted all-time — WR {c.get('win_rate',0):.1f}% · "
                                f"EVw {c.get('ev_weighted',0):+.2f}R · "
                                f"PF {c.get('pf',0):.2f} · n={c.get('n',0)}")
                ...
                st.markdown(
                    ...
                    f'🧠 Step 2 — Choose a Method &amp; Train ML</div>'
                    f'<div style="color:#8892b0;font-size:11px;">'
                    f'Train an adaptive classifier (LR / RF / GB auto-picked by sample size) '
                    f'on historical trades labeled by the chosen method. You can train both '
                    f'candidates and compare them side-by-side.</div></div>',
                    unsafe_allow_html=True,
                )

                _ml_col_a, _ml_col_b = st.columns(2)
                with _ml_col_a:
                    st.markdown(
                        f'<div style="background:#0d1f0d;border:1px solid #238636;'
                        f'border-radius:6px;padding:8px 10px;margin-bottom:6px;">'
                        f'<div style="color:#3fb950;font-size:10px;font-weight:700;'
                        f'text-transform:uppercase;">🟢 Candidate A — Best in Newest Bucket</div>'
                        f'<div style="color:#ccd6f6;font-size:11px;font-family:monospace;'
                        f'margin-top:3px;">{_cand_label(_cand_a_dict)}</div>'
                        f'<div style="color:#8892b0;font-size:10px;margin-top:2px;">'
                        f'{_cand_stats_line(_cand_a_dict, "A")}</div></div>',
                        unsafe_allow_html=True,
                    )
                    if st.button("🧠 Train ML on Candidate A",
                                 key=f"mlA_btn_{_sym_key}_{i}",
                                 use_container_width=True,
                                 disabled=(_cand_a_dict is None)):
                        with st.spinner(f"Training ML on Candidate A…"):
                            _ml_a = _scanner_train_ml(sig, _cand_a_dict["method_cfg"])
                        st.session_state[_ml_a_key]   = _ml_a
                        st.session_state[_ml_primary] = "A"
                        st.session_state[_ml_cache_key] = _ml_a
                        st.session_state.pop(_ai_key, None)

                with _ml_col_b:
                    st.markdown(
                        f'<div style="background:#0a1628;border:1px solid #1f6feb;'
                        f'border-radius:6px;padding:8px 10px;margin-bottom:6px;">'
                        f'<div style="color:#58a6ff;font-size:10px;font-weight:700;'
                        f'text-transform:uppercase;">🔵 Candidate B — Weighted All-Time Best</div>'
                        f'<div style="color:#ccd6f6;font-size:11px;font-family:monospace;'
                        f'margin-top:3px;">{_cand_label(_cand_b_dict)}</div>'
                        f'<div style="color:#8892b0;font-size:10px;margin-top:2px;">'
                        f'{_cand_stats_line(_cand_b_dict, "B")}</div></div>',
                        unsafe_allow_html=True,
                    )
                    if st.button("🧠 Train ML on Candidate B",
                                 key=f"mlB_btn_{_sym_key}_{i}",
                                 use_container_width=True,
                                 disabled=(_cand_b_dict is None)):
                        with st.spinner(f"Training ML on Candidate B…"):
                            _ml_b = _scanner_train_ml(sig, _cand_b_dict["method_cfg"])
                        st.session_state[_ml_b_key]   = _ml_b
                        st.session_state[_ml_primary] = "B"
                        st.session_state[_ml_cache_key] = _ml_b
                        st.session_state.pop(_ai_key, None)

                # If both A and B have been trained, offer a radio to pick which
                # the display + AI verdict should use
                _has_a = _ml_a_key in st.session_state
                _has_b = _ml_b_key in st.session_state
                if _has_a and _has_b:
                    _current_primary = st.session_state.get(_ml_primary, "A")
                    _pick = st.radio(
                        "Display / AI source:",
                        options=["A", "B"],
                        index=0 if _current_primary == "A" else 1,
                        horizontal=True,
                        key=f"ml_pick_{_sym_key}_{i}",
                        format_func=lambda x: f"🟢 Candidate A" if x == "A" else f"🔵 Candidate B",
                    )
                    if _pick != _current_primary:
                        st.session_state[_ml_primary] = _pick
                        st.session_state[_ml_cache_key] = (
                            st.session_state[_ml_a_key] if _pick == "A"
                            else st.session_state[_ml_b_key]
                        )
                        st.session_state.pop(_ai_key, None)

            # ── Step 3: AI Final Verdict ─────────────────────────────────────
            _ml_ready = _ml_cache_key in st.session_state
            _ai_disabled = not _has_ai_key or not (_bt_ready and _ml_ready)
            _ai_tip = (
                "Run Step 1 + train at least one ML candidate first."
            ...
            if st.button("🤖 Step 3 — AI Final Verdict",
            ...
            if _bt_res or _ml_res:
                ...
                if _ml_a_show or _ml_b_show:
                    _header = (
                        ...
                        f'{" (showing: Candidate " + _ml_primary_show + ")" if (_ml_a_show and _ml_b_show) else ""}</div>'
                        f'</div>'
                    )
                ...
                st.markdown(
                    ...
                    '▸ Click <b>Step 1</b> (Backtest + WFO) → pick a Candidate and <b>Train ML</b> '
                    '→ click <b>Step 3</b> for AI Final Verdict.</div>',
                    unsafe_allow_html=True,
                )
