...
def _scanner_mini_wfo(sig: dict, bt_results: dict) -> dict:
    ...
    if not best_key or best.get("insufficient"):
        return {
            ...
            "note":        "Backtest tidak menemukan metode yang valid (butuh ≥ 4 trades). WFO tidak dapat dijalankan.",
        }
    ...
    if df.empty or len(df) < 60:
        return {
            ...
            "note":        "< 60 bar tersedia — tidak cukup data historis. WFO diabaikan.",
        }
    ...
    if len(df_is) < 30 or len(df_oos) < 15:
        return {
            ...
            "verdict":     "INSUFFICIENT",
            "method_used": best_key,
            "note":        "Bar tidak cukup untuk split IS/OOS. WFO diabaikan.",
        }
    ...
    if is_n < 3:
        return {
            ...
            "note":         f"Hanya {is_n} IS trades — tidak cukup untuk validasi statistik. WFO diabaikan, gunakan sinyal dengan hati-hati.",
        }
      ...
      <div style="color:#ccd6f6;">Price already >1R from aggressive entry without a retrace</div>
            ...
            if _bt_res or _ml_res:
                ...
                if _wfo_res:
                    ...
                    if _wvo_ran := _wfo_ran and _wv != "INSUFFICIENT":
                        ...
                        _wfo_block_html = (
                            ...
                            f'<div style="color:#ccd6f6;font-size:14px;font-weight:800;">{_wfo_res.get("is_pf",0):.2f}</div>'
                            ...
                            f'<div style="color:{_wv_col};font-size:14px;font-weight:800;">{_wfo_res.get("oos_pf",0):.2f}</div>'
                        ...
                        _wfo_block_html = (
                            ...
                            f'🔬 WFO Mini-Validation — ⚠️ TIDAK CUKUP SAMPEL</div>'
                            f'<div style="color:#ccd6f6;font-size:12px;margin-bottom:4px;">'
                            f'Method yang dicoba: <b>{_wfo_meth}</b>'
                            ...
                            f'Hasil WFO diabaikan — sinyal ini tetap bisa dipertimbangkan berdasarkan backtest dan ML.</div>'
                            f'</div>'
                        )
                ...
                _layer3_oi  = (f"OI 24h: {_l3_oi_val:+.1f}%" if _l3_oi_val is not None
                               else "OI 24h: N/A (spot only atau API belum tersedia)")
                ...
                ) if _bk_disp != "—" else "Backtest: tidak ada metode yang valid"
                ...
                if _wfo_l.get("ok"):
                    ...
                    if _wfo_v == "INSUFFICIENT":
                        _layer6 = (
                            ...
                            f"tidak cukup sampel, hasil diabaikan | Method: {_wfo_l.get('method_used','—')}"
                        )
                    else:
                        _layer6 = (
                            f"WFO: {_wfo_v} | "
                            f"IS PF={_wfo_l.get('is_pf',0):.2f} (n={_wfo_l.get('is_n',0)}) | "
                            f"OOS PF={_wfo_l.get('oos_pf',0):.2f} WR={_wfo_l.get('oos_wr',0):.1f}% "
                    ...
                    _layer6 = f"WFO: tidak bisa dijalankan — {_wfo_l.get('note', 'data tidak cukup')}"
                else:
                    _layer6 = "WFO: belum dijalankan (klik Step 1 terlebih dahulu)"
