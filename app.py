...
def calculate_adx(df: pd.DataFrame, period: int = 14) -> pd.Series:
    """Calculate ADX (Average Directional Index) from OHLCV DataFrame.
    Returns a Series of ADX values aligned to df.index."""
    high  = df["high"]
    low   = df["low"]
    close = df["close"]
...
def detect_qualifying_candles(df, min_body_pct=0.70, min_vol_mult=1.5,
    ...
    """Return qualifying momentum candles, optionally filtered by ADX threshold.

    adx_series: pre-computed ADX aligned to df.index (can be from a higher TF).
    """
    mask = (
        (df["body"] > 0) &
    ...
    if adx_filter and adx_series is not None:
        # Align ADX to df index (forward-fill gaps from higher TF)
    ...
    if adx_filter and adx_series is not None:
        adx_aligned = adx_series.reindex(df.index, method="ffill")
        result["adx_value"] = adx_aligned[mask].round(1)
    else:
...
def simulate_trade(
    ...
    if tp_mode == "2R":
        ...
        tp2 = None
    else:  # Partial: half out at 1R, full exit at 2R
        tp1 = entry_price + 1 * risk
        tp2 = entry_price + 2 * risk

    future_start = trigger_idx + 1
    # Use a fixed 30-bar window to search for entry fill, regardless of max_hold_bars.
    # This prevents short max_hold_bars values from cutting off the entry search
    # before late-filling retracement entries are found.
    # Once entry fills, future_end is re-set to entry_bar_i + 1 + max_hold_bars.
    ENTRY_SEARCH_BARS = 30
    future_end   = min(future_start + ENTRY_SEARCH_BARS, len(df))
    if future_start >= len(df):
        return None

    # For 0% retracement (immediate entry), entry is on the trigger candle itself
    if retracement == 0.0:
        entry_filled = True
        entry_date   = df.index[trigger_idx]
        entry_bar_i  = trigger_idx
        # Hold window starts from trigger+1, length = max_hold_bars
    ...
    for fwd_i in range(scan_start, future_end):
        ...
        if not entry_filled:
            if bar["low"] <= entry_price:
                ...
                entry_date   = bar_date
                entry_bar_i  = fwd_i
                # Extend the scan window so the hold is always max_hold_bars
                # candles counted FROM entry, not from the trigger.
        ...
        # ── Time exit: close at open of the last allowed bar ────────────────
        if use_time_exit and fwd_i == future_end - 1:
            ep = float(bar["open"])

            # Gap check: if opening price already crossed TP or SL,
            # treat it as a TP/SL hit rather than a time exit.
            # This prevents absurdly high R from overnight gap-ups.
            if ep <= current_sl:
                r = -(1 + transaction_cost * 2)
                return _trade_dict(trigger, df.index[trigger_idx], entry_price,
                                   entry_date, sl_price, tp1, current_sl,
                                   ...
                                   sl_dist, tp_mode, sl_mode,
                                   exit_type="SL",
                                   candles_held=fwd_i - entry_bar_i)
            if ep >= tp1:
            ...
            # Normal time exit — open price is between SL and TP
            if tp_mode == "Partial" and partial_exited:
                ...
                r   = (ep - entry_price) / risk - transaction_cost * 2
    ...
    # Post-loop: entry filled but no TP/SL within max_hold_bars
    # (use_time_exit=True exits within the loop; this handles use_time_exit=False
    #  or the edge case where entry fills on the very last scanned bar)
...
def run_backtest(df, qualifying, retracement, sl_dist, tp_mode,
                 ...
                 max_hold_bars=5, use_time_exit=False) -> List[Dict]:
    trades   = []
...
def optimize(df, qualifying, sl_mode="Fixed", trail_atr_mult=1.5,
    ...
    IMPORTANT: optimization always runs with use_time_exit=False and a large
    max_hold_bars so the parameter sweep reflects genuine TP/SL performance,
    not selection bias from an arbitrary time cut-off.  The sidebar's time exit
    settings are applied separately when displaying the best-setup trades.
    """
    results    = []
    all_trades = {}
    ...
    for i, (ret, sl, tp) in enumerate(combos):
        # Always use unlimited hold / no time-exit for the optimization sweep
        trades = run_backtest(df, qualifying, ret, sl, tp,
                              sl_mode, trail_atr_mult, transaction_cost,
                              max_hold_bars=500, use_time_exit=False)
...
def analyze_hold_times(df, qualifying, ret, sl_dist, tp_mode,
    ...
    for n in range(1, 21):
        trades = run_backtest(df, qualifying, ret, sl_dist, tp_mode,
                              sl_mode, trail_mult, tc,
                              max_hold_bars=n, use_time_exit=True)
        m     = calc_metrics(trades)
...
def plot_candle_resolution(df, qualifying, ret, sl_dist, tp_mode,
    ...
    Uses max_hold_bars=500 (no cap) so every trade eventually closes via TP or SL.
...
— signal is triggered by <b>Body %</b> + <b>Volume</b> alone.
ADX just tells you whether the market is trending or ranging at the time of the signal.<br>
...
&nbsp;({timeframe} → {htf} for ADX context)
    ...
    adx_signal = calculate_adx(live_df) if not live_df.empty else pd.Series(dtype=float)
    adx_htf_raw = calculate_adx(live_df_htf) if not live_df_htf.empty else pd.Series(dtype=float)
    ...
    for i, (idx, row) in enumerate(scan.iterrows()):
        ...
        # Alignment: both ≥ 25 = trending, both < 25 = ranging, mixed = diverging
        if pd.notna(adx_s_val) and pd.notna(adx_h_val):
            if adx_s_val >= 25 and adx_h_val >= 25:
                adx_align = "✅ Both trending"
            elif adx_s_val < 25 and adx_h_val < 25:
    ...
    display_cols = ["#", "Date", "Body %", "Vol Mult", "Body OK", "Vol OK",
                    "SIGNAL", adx_s_col, adx_h_col, "ADX Align"]
    hidden_cols  = ["_body_ok", "_vol_ok", "_signal", "_adx_s", "_adx_h"]
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Body %":      st.column_config.NumberColumn("Body %",    width=75,  format="%.1f"),
            "Vol Mult":    st.column_config.NumberColumn("Vol×",       width=65,  format="%.2f"),
            "Body OK":     st.column_config.TextColumn("Body",        width=55),
            "Vol OK":      st.column_config.TextColumn("Vol",         width=50),
            "SIGNAL":      st.column_config.TextColumn("Signal",      width=75),
            adx_s_col:     st.column_config.TextColumn(adx_s_col,     width=80,
                               help=f"ADX(14) on the {tf_label} timeframe. ≥25 = trending"),
            adx_h_col:     st.column_config.TextColumn(adx_h_col,     width=90,
                               help=f"ADX(14) on the {htf_label} timeframe. ≥25 = trending"),
            "ADX Align":   st.column_config.TextColumn("ADX Align",   width=140,
                               help="Both trending (≥25) = stronger signal context"),
        },
    )
...
<div class="signal-line">ADX ({tf_label}): <span style="color:{adx_s_color};">{adx_s_disp}</span>
&nbsp;|&nbsp; ADX ({htf_label} HTF): <span style="color:{adx_h_color};">{adx_h_disp}</span>
...
def main():
    with st.sidebar:
        ...
        if adx_filter:
            ...
            st.caption(
                ...
                "Filtering for trending markets can improve signal quality."
            )
    ...
    adx_series_signal = calculate_adx(df) if adx_filter else None
    ...
    if adx_filter and adx_tf_mode in ("One timeframe higher", "Both"):
        ...
        if higher_tf != timeframe:
            ...
            if not df_higher.empty:
                adx_series_higher = calculate_adx(df_higher)
        else:
    ...
    run_key = (f"{ticker}_{timeframe}_{history_label}_{min_body_pct:.2f}_"
               ...
               f"adx{adx_filter}_{adx_threshold}_{adx_tf_mode}")
        ...
        with col_l:
            ...
            if adx_filter:
                adx_badge = (f'<div style="color:#ffd700;font-size:13px;margin-top:6px;">'
                             ...
                             f'({adx_tf_mode})</div>')
            st.markdown(f"""<div style="font-size:36px;font-weight:800;color:#64ffda;margin-bottom:4px;">
...
</div>{adx_badge}""", unsafe_allow_html=True)
        ...
        for ret in RETRACEMENTS:
            ...
            for (r, sl, tp), trades in all_trades.items():
                if r != ret:
                    continue
                for t in trades:
                    # Only include genuine TP or SL exits — skip Max Hold / Time Exit
                    ...
                    trade_bars = df.loc[df_reset[mask].iloc[:, 0].values] if mask.any() else pd.DataFrame()
                    if trade_bars.empty:
                        continue
                    lowest_price = trade_bars["low"].min()
                    mae_r = (t["entry_price"] - lowest_price) / risk
                    vals.append(max(0.0, min(mae_r, 1.5)))  # cap at 1.5R for display sanity
