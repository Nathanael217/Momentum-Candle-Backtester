...
def render_pulse_tab():
    ...
    Phase 1: DefiLlama TVL only. Phase 2 will add exchange flow,
    social pulse, and smart money proxy.
    """
    ...
    st.markdown(
        ...
        '<b style="color:#3fb950;">Phase 1 (now):</b> DefiLlama TVL tracker — works for '
        'all major DeFi tokens (AAVE, UNI, LDO, GMX, etc.) and L1 chains (ETH, SOL, AVAX). '
        '<br><b style="color:#8892b0;">Phase 2 (next):</b> Etherscan exchange flow, '
        'LunarCrush social pulse, Solscan flows, smart money proxy.</div>',
        unsafe_allow_html=True,
    )
    ...
    if not analyze_clicked and not st.session_state.get("pulse_last_result"):
        st.info(
            ...
            "Pulse currently tracks ~30 DeFi tokens and ~20 L1 chains. "
            "Tokens like BTC/DOGE/PEPE return N/A in Phase 1 (Phase 2 covers them)."
        )
        return
    ...
    if analyze_clicked:
        with st.spinner(f"Fetching on-chain data for {symbol}..."):
            try:
                result = _pulse.get_pulse_intel(symbol)
    ...
    # ── Phase 2 placeholders so the UI shows what's coming ──────────────────
    for emoji, name, desc in [
        ("💸", "Exchange Flow",  "Net inflow vs outflow to centralized exchanges (Etherscan)"),
        ...
        st.markdown(
            ...
            f'padding:1px 8px;border-radius:4px;font-size:10px;">PHASE 2 — COMING SOON</span>'
            f'</div>'
