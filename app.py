...
def _scanner_ai_verdict(sig: dict, ml: dict = None, bt: dict = None, wfo: dict = None) -> dict:
    ...
    if wfo and wfo.get("ok"):
        ...
        wfo_section  = (
            ...
            f"IS: PF={_wfo_is_pf:.2f} n={_wfo_n_is} | OOS: PF={_wfo_oos_pf:.2f} WR={_wfo_oos_wr:.1f}% n={_wfo_n_oos}\\n"
...
def _scanner_mini_wfo(sig: dict, bt_results: dict) -> dict:
    ...
    Uses the BEST method found by _scanner_quick_backtest on the IS (first 70%)
    window, then re-runs it on the OOS (last 30%) window.

    Returns dict:
      ok, verdict (PASS/FAIL/INSUFFICIENT), is_pf, oos_pf, oos_wr, oos_n,
      is_n, oos_is_ratio, method_used, tier_label, note
    """
    import math
    ...
    if df.empty or len(df) < 60:
        return {"ok": False, "verdict": "INSUFFICIENT", "note": "< 60 bars available"}

    # ── Best method from backtest ─────────────────────────────────────────────
    best     = bt_results.get("best", {})
    best_key = bt_results.get("best_key", "")
    if not best_key or best.get("insufficient"):
        return {"ok": False, "verdict": "INSUFFICIENT", "note": "No valid method from backtest"}
    ...
    if len(df_is) < 30 or len(df_oos) < 15:
        return {"ok": False, "verdict": "INSUFFICIENT", "note": "Not enough bars for IS/OOS split"}
    ...
    def _pf(ts):
        ...
        gp = sum(wins)
        gl = abs(sum(losses))
        return round(gp / gl, 3) if gl > 0 else (99.0 if gp > 0 else 0.0)
    ...
    if is_n < 5:
        return {"ok": False, "verdict": "INSUFFICIENT",
                "note": f"Only {is_n} IS trades (need ≥ 5)"}

    oos_is_ratio = round(oos_pf / is_pf, 3) if is_pf > 0 else 0.0
      ...
      <div style="color:#ccd6f6;">Price already >1R from aggressive entry without a retrace</div>
            ...
            with _step1_col:
                if st.button("📊 Step 1 — Backtest + ML + WFO",
                             ...
                             use_container_width=True,
                             help="Scan 500 bars historically + compute ML probability + WFO mini-validation. Run this first."):
                    with st.spinner("Backtesting + ML + WFO…"):
            ...
            if _bt_res or _ml_res:
                ...
                if _wfo_res:
                    ...
                    _wv_icon = {"PASS": "✅", "BORDERLINE": "⚠️",
                                "FAIL": "❌", "INSUFFICIENT": "—"}.get(_wv, "—")
                    _wfo_block_html = (
                        f'<div style="margin-top:10px;padding:10px 14px;{_wv_bg and "background:" + _wv_bg + ";"}'
                        f'border:1px solid {_wv_col};border-radius:8px;">'
                        ...
                        f'<div style="color:#ccd6f6;font-size:14px;font-weight:800;">'
                        f'{_wfo_res.get("is_pf",0):.2f}</div>'
                        ...
                        f'<div style="color:{_wv_col};font-size:14px;font-weight:800;">'
                        f'{_wfo_res.get("oos_pf",0):.2f}</div>'
                        ...
                        f'<div style="color:#ccd6f6;font-size:14px;font-weight:800;">'
                        f'{_wfo_res.get("oos_wr",0):.1f}%</div></div>'
                        f'<div style="background:#0a0f1a;border-radius:4px;padding:5px 8px;">'
                        f'<div style="color:#8892b0;font-size:9px;text-transform:uppercase;">OOS/IS Ratio</div>'
                        f'<div style="color:#ccd6f6;font-size:14px;font-weight:800;">'
                        f'{_wfo_res.get("oos_is_ratio",0):.2f}</div></div>'
                        f'</div>'
                        f'<div style="color:#8892b0;font-size:10px;">'
                        f'Method: {_wfo_res.get("method_used","—")} &nbsp;|&nbsp; '
                        f'{_wfo_res.get("tier_label","70% IS / 30% OOS")}</div>'
                        f'<div style="color:{_wv_col};font-size:11px;margin-top:4px;">'
                        f'{_wfo_res.get("note","")}</div>'
                        f'</div>'
                    )

                # ── 6 Intelligence Layers Expander ────────────────────────────
                # Pre-compute all layers for display + AI cross-check
                _layer1 = (
                ...
                _layer2_btcd = f"BTC Dominance: {sig.get('btc_dominance', 'N/A'):.1f}%" if sig.get("btc_dominance") else "BTC Dominance: N/A"
                _layer2_fng  = f"F&G: {sig.get('fng_value', 'N/A')}" if sig.get("fng_value") else "F&G: N/A"
                _layer2 = f"{_layer2_btcd} | {_layer2_fng}"
                _layer3_oi  = (f"OI 24h: {sig.get('oi_change_pct', 'N/A'):+.1f}%"
                               if sig.get("oi_change_pct") is not None else "OI 24h: N/A")
                _layer3_fr  = (f"Funding: {sig.get('funding_rate', 0)*100:.4f}%"
                               if sig.get("funding_rate") is not None else "Funding: N/A")
                _layer3_tbr = f"Taker Buy: {sig.get('taker_buy_ratio', 0.5)*100:.1f}%"
                ...
                _wfo_l = _wfo_res or {}
                _layer6 = (
                    f"WFO: {_wfo_l.get('verdict','—')} | "
                    f"IS PF={_wfo_l.get('is_pf',0):.2f} | "
                    f"OOS PF={_wfo_l.get('oos_pf',0):.2f} | "
                    f"OOS WR={_wfo_l.get('oos_wr',0):.1f}% | "
                    f"Ratio={_wfo_l.get('oos_is_ratio',0):.2f}"
                ) if _wfo_l.get("ok") else "WFO: not run yet (run Step 1)"
                ...
                for _zn in ("Aggressive", "Standard", "Sniper"):
                    ...
                    if _zn == "Standard" and not _etp_card.get("std_valid", True):
