      ...
      <div style="color:#ccd6f6;">Price already >1R from aggressive entry without a retrace</div>
            ...
            if _bt_res or _ml_res:
                ...
                for _zn in ("Aggressive", "Standard", "Sniper"):
                    ...
                    _border = "border:1px solid #3fb950;" if _is_best_zone else "border:1px solid #21262d;"
                    _crown  = " 👑 BEST" if _is_best_zone else ""
                    if _zd and not _zd.get("insufficient"):
                        _zone_table_rows += (
                            f'<div style="background:#0d1117;{_border}border-radius:6px;padding:8px 10px;margin-bottom:6px;">'
                            f'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">'
                            f'<span style="color:#ccd6f6;font-size:12px;font-weight:700;">{_zone_icons.get(_zn,"•")} {_zn}{_crown}</span>'
                            f'<span style="color:#8892b0;font-size:10px;">{_zd.get("sl_label","?")} · {_zd.get("mgmt","?")} · n={_zd.get("n",0)}</span>'
                            f'</div>'
                            f'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px;">'
                            f'<div><div style="color:#8892b0;font-size:10px;">Win Rate</div>'
                            f'<div style="color:{_wr_color(_zd.get("win_rate",0))};font-size:13px;font-weight:700;">{_zd.get("win_rate",0):.1f}%</div></div>'
                            f'<div><div style="color:#8892b0;font-size:10px;">Exp. Value</div>'
                            f'<div style="color:{_ev_color(_zd.get("ev",0))};font-size:13px;font-weight:700;">{_zd.get("ev",0):+.2f}R</div></div>'
                            f'<div><div style="color:#8892b0;font-size:10px;">Avg Hold</div>'
                            f'<div style="color:#ccd6f6;font-size:13px;font-weight:700;">{_zd.get("avg_bars",0):.1f}b</div></div>'
                            f'</div></div>'
                        )
                    else:
                        _zone_table_rows += (
                            ...
                            f'<span style="color:#8892b0;font-size:12px;">{_zone_icons.get(_zn,"•")} {_zn} — insufficient data</span>'
                            f'</div>'
                        )
                ...
                if _best and _best_key:
                    ...
                    _bn    = _best.get("n", 0)
                    _bzone = _best.get("zone", "?")
                    _bsl   = _best.get("sl_label", "?")
                    _bmgmt = _best.get("mgmt", "?")
                    _recommendation_html = (
                        ...
                        f'letter-spacing:1px;font-weight:700;margin-bottom:6px;">🏆 Best Proven Method</div>'
                        f'<div style="color:#ccd6f6;font-size:13px;font-weight:700;margin-bottom:4px;">{_best_key}</div>'
                        f'<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;margin-top:6px;">'
                        f'<div><div style="color:#8892b0;font-size:10px;">Win Rate</div>'
                        f'<div style="color:{_wr_color(_bwr)};font-size:14px;font-weight:800;">{_bwr:.1f}%</div></div>'
                        f'<div><div style="color:#8892b0;font-size:10px;">Exp. Value</div>'
                        f'<div style="color:{_ev_color(_bev)};font-size:14px;font-weight:800;">{_bev:+.2f}R</div></div>'
                        f'<div><div style="color:#8892b0;font-size:10px;">Sample</div>'
                        f'<div style="color:#ccd6f6;font-size:14px;font-weight:800;">{_bn} trades</div></div>'
                        f'</div></div>'
                    )
                else:
                    _recommendation_html = (
                        f'<div style="color:#8892b0;font-size:12px;padding:5px 0;">'
                        f'Not enough data to determine best method.</div>'
                    )
