    ...
    for _ratchet in _RATCHET_RATIOS:
        ...
        for i in range(14, n_df - 2):
                ...
                feat = [
                    ...
                    # NEW: body size normalized to ATR — captures explosiveness
                    float(bar.get("body_vs_atr", 0.0) or 0.0),
                    # NEW: stretch from EMA21 — signed %, mean-reversion risk indicator.
...
def render_auto_analyzer(ticker: str, df_full_1d: pd.DataFrame, tc: float,
                          current_tf: str, manual_sig: dict = None):
    """
    ...
    When `manual_sig` is provided, this function skips the scanner UI (no
    universe scan, no controls, no timeframe selector) and renders the full
    detailed signal card for just that one pre-built sig. This is how the
    Manual Analyzer tab reuses the exact same rendering code as the Scanner
    tab — same format, same depth, same execution plan, same AI verdict.
    """
    import concurrent.futures

    _manual_mode = manual_sig is not None
    ...
    # When a pre-built sig is passed (from the Manual Analyzer tab), skip the
    # entire scanner universe scan. Set the manual-mode session state so the
    # render block picks it up instead of the scanner results.
    if _manual_mode:
        st.session_state["_manual_render_signals"] = [manual_sig]
        st.session_state["_manual_render_scanned_at"] = manual_sig.get(
            "candle_date", "manual analysis"
        )
...
def render_pulse_tab():
    ...
    # ── API Keys ─────────────────────────────────────────────────────────────
    # Three independent keys — each module degrades gracefully if its key is
    # missing (shows "NO KEY" badge on the relevant card).
    ...
    _any_missing = not (_have_es and _have_lc and _have_ss)

    with st.expander("🔑 API Keys (Etherscan + LunarCrush + Solscan)",
                     expanded=_any_missing):
        st.markdown(
            ...
            '<b style="color:#58a6ff;">Free keys used by Pulse:</b><br>'
            '• <b>Etherscan</b>: etherscan.io/apis — ERC-20 CEX flow (5/sec, 100k/day)<br>'
            '• <b>LunarCrush</b>: lunarcrush.com/developers — Galaxy score + sentiment<br>'
            '• <b>Solscan</b>: pro-api.solscan.io — SPL-token CEX flow (Pro tier has free plan)<br>'
            '<b>TVL + macro backdrop work without any keys.</b> Each module shows "NO KEY" '
            'if its key is missing — nothing breaks, you just lose that card.'
            '</div>',
            unsafe_allow_html=True,
        )
        ...
        with col_k1:
            ...
            st.caption("✅ Set" if _have_es else "⚠️ Missing")
        with col_k2:
            st.text_input(
                "LunarCrush Key", type="password",
                key="pulse_lunarcrush_key",
                placeholder="Bearer token...",
                help="Free at lunarcrush.com/developers",
            )
            st.caption("✅ Set" if _have_lc else "⚠️ Missing")
        with col_k3:
            st.text_input(
                "Solscan Pro Key", type="password",
                key="pulse_solscan_key",
                placeholder="eyJ... or your Solscan token",
                help="Free at pro-api.solscan.io",
            )
            st.caption("✅ Set" if _have_ss else "⚠️ Missing")
    ...
    # ── Placeholder for the one remaining future module ─────────────────────
    st.markdown(
        '<div style="background:#0d1117;border:1px dashed #30363d;border-radius:8px;'
        'padding:10px 14px;margin-top:8px;opacity:0.6;">'
        '<div style="display:flex;justify-content:space-between;align-items:center;">'
        '<div style="color:#8892b0;font-size:11px;text-transform:uppercase;'
        'letter-spacing:1px;font-weight:700;">🐋 Smart Money Proxy</div>'
        '<span style="color:#8892b0;border:1px solid #30363d;'
        'padding:1px 8px;border-radius:4px;font-size:10px;">PHASE 4 — FUTURE</span>'
        '</div>'
        '<div style="color:#8892b0;font-size:11px;margin-top:4px;">'
        'Large whale swaps on Uniswap v3 / PancakeSwap / Raydium via The Graph subgraphs. '
        'Not a true wallet-labeled "smart money" (that\'s Nansen\'s moat) but the closest '
        'free approximation — filters single swaps above $100K USD.'
        '</div>'
        '</div>',
        unsafe_allow_html=True,
    )
...
def render_manual_analyzer_tab():
    ...
    with col_d2:
        if _needs_time:
            # For hourly frames we need hour selection. Allowed hours depend on TF.
            _tf_hours = {"1H": 1, "2H": 2, "4H": 4, "6H": 6, "12H": 12}[timeframe]
            _valid_hours = list(range(0, 24, _tf_hours))
            default_hour = st.session_state.get("manual_last_hour", 0)
            ...
            selected_hour = 0   # daily candles open at 00:00 UTC
    ...
    if analyze_clicked:
        ...
        with st.spinner(f"Fetching {symbol} {timeframe} data and scoring candle..."):
            try:
                ...
                # Build signal with RELAXED filters (min_body=0, min_vol=0) so
                # the scorer never rejects the user's chosen bar — we want to
                # analyze whatever they picked, even small-body candles.
                sig = _scanner_score_signal(
                    df, adx_df, bar_idx, direction,
                    timeframe, symbol,
                    min_body_pct=0.0,
                ...
                if sig is None:
                    st.error(
                        f"Could not score this candle — it may not match the requested "
                        f"direction (bullish bar needed for long, bearish for short), "
                        f"or the bar's data is corrupt. Try the other direction."
                    )
                    return
                ...
                _ts_wib = _ts_utc + pd.Timedelta(hours=7)
                sig["candle_date"] = _ts_wib.strftime("%Y-%m-%d %H:%M WIB")
                sig["_manual_mode"] = True   # marker so UI can differentiate
    ...
    if st.button("📊 Step 1 — Backtest + WFO (deep historical scan)",
                 ...
                 help="Runs 72 method combinations + purged WFO. Takes ~10-30 sec."):
        with st.spinner("Backtesting 72 methods + WFO validation..."):
            _bt = _scanner_quick_backtest(sig)
            _wfo = _scanner_mini_wfo(sig, _bt)
    ...
    if _ml_a and _ml_b:
        if st.button("🤖 Step 3 — AI Dual-Candidate Verdict",
                     ...
                     help="Sends full context (backtest + WFO + ML + onchain) to AI for final verdict."):
