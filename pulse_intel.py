    ...
    if base not in _SYMBOL_TO_SPL_MINT:
        return {
            ...
            "data": {"mint": "", "n_transfers": 0,
                     "n_cex_inflows": 0, "n_cex_outflows": 0,
                     "inflow_usd": 0, "outflow_usd": 0, "net_flow_usd": 0,
                     "window_hours": 0, "needs_api_key": False},
        }
    ...
    if not api_key:
        return {
            ...
            "data": {"mint": _SYMBOL_TO_SPL_MINT[base], "n_transfers": 0,
                     "n_cex_inflows": 0, "n_cex_outflows": 0,
                     "inflow_usd": 0, "outflow_usd": 0, "net_flow_usd": 0,
                     "window_hours": 0, "needs_api_key": True},
        }
    ...
    if cex_total == 0:
        return {
            ...
            "data": {"mint": mint, "n_transfers": len(transfers),
                     "n_cex_inflows": 0, "n_cex_outflows": 0,
                     "inflow_usd": 0, "outflow_usd": 0, "net_flow_usd": 0,
                     "window_hours": window_hours, "needs_api_key": False},
        }
...
def get_pulse_intel(symbol: str,
    ...
    Phase 3 (current): TVL + Etherscan Flow + Solscan Flow + LunarCrush Social
                       + Macro backdrop modifier.
    ...
    Composite weighting (per-token, before macro modifier):
      When modules are available, the following weights are used:
        - Exchange Flow (ETH or SOL, whichever applies): 40%
        - TVL:                                           35%
        - Social:                                        25%
    ...
    # Base weights: Flow 40%, TVL 35%, Social 25%
    ...
    # Agreement/divergence detection (only across per-token signals)
    _pt_dirs = []
    if tvl.get("ok") and tvl.get("supported"):
    ...
    phase_label = f"3 (TVL + {flow_chain} Flow + Social + Macro)" if active_flow else "3 (limited modules available)"
