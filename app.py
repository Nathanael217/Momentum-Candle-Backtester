...
def _scanner_score_signal(
    ...
    if adx_df is not None and not adx_df.empty and bar_idx < len(adx_df):
        try:
            adx_val  = float(adx_df["adx"].iloc[bar_idx])
            di_plus  = float(adx_df["di_plus"].iloc[bar_idx])
            di_minus = float(adx_df["di_minus"].iloc[bar_idx])
        except Exception:
            pass
    ...
    # ── Entry levels ──────────────────────────────────────────────────────────
    # Use 0% retrace (enter at close) + 1.5% SL + 2R TP as universal defaults
    sl_dist = 0.015
    if close_px > 0:
        if direction == "long":
            entry  = round(close_px, 8)
            sl     = round(entry * (1 - sl_dist), 8)
            tp2r   = round(entry + 2 * (entry - sl), 8)
            tp3r   = round(entry + 3 * (entry - sl), 8)
        else:
            entry  = round(close_px, 8)
            sl     = round(entry * (1 + sl_dist), 8)
            tp2r   = round(entry - 2 * (sl - entry), 8)
            tp3r   = round(entry - 3 * (sl - entry), 8)
    else:
        entry = sl = tp2r = tp3r = 0.0
...
def _scanner_ai_verdict(sig: dict, ml: dict = None, bt: dict = None) -> dict:
    """
    Call Groq LLM to synthesize all available evidence (signal + ML + backtest).
    Returns dict with: verdict, confidence, rationale, risk, source.
    """
    api_key = st.session_state.get("groq_api_key", "")
    if not api_key:
        return {
...
def _scanner_quick_backtest(sig: dict) -> dict:
    """
    Fetch up to 500 bars for this signal's coin/timeframe and run a quick
    historical backtest: how often does a candle matching these criteria
    hit 2R (3%) or 3R (4.5%) before the 1.5% SL?
    Returns a dict of stats.
    """
    symbol    = sig["symbol"]
    timeframe = sig["timeframe"]
    direction = sig["direction"]
    # Use 70% of the signal's own thresholds so we get enough sample trades
    min_body = abs(sig["body_pct"]) * 0.70 / 100   # convert % back to fraction
    ...
    for i in range(14, n - 2):
        ...
        for j in range(i + 1, min(i + MAX_HOLD + 1, n)):
            ...
            if hit_sl:
                res2 = res3 = "SL"
                break
            if hit_tp3 and res3 == "OPEN":
                res3 = "TP"
            if hit_tp2 and res2 == "OPEN":
                res2 = "TP"
            if res2 != "OPEN" and res3 != "OPEN":
                break

        trades.append({"r2": res2, "r3": res3, "bars": bars_held})

    if not trades:
        return {"error": "No matching historical setups found", "n": 0}

    c2  = [t for t in trades if t["r2"] in ("TP", "SL")]
    c3  = [t for t in trades if t["r3"] in ("TP", "SL")]
    w2  = len([t for t in c2 if t["r2"] == "TP"]) / len(c2) if c2 else 0
    w3  = len([t for t in c3 if t["r3"] == "TP"]) / len(c3) if c3 else 0
    avg_bars = sum(t["bars"] for t in trades) / len(trades)
    ev2 = round(w2 * 2 - (1 - w2) * 1, 2)
    ev3 = round(w3 * 3 - (1 - w3) * 1, 2)

    return {
        "n":          len(trades),
        "win_2r":     round(w2 * 100, 1),
        "win_3r":     round(w3 * 100, 1),
        "ev_2r":      ev2,
        "ev_3r":      ev3,
...
def render_auto_analyzer(ticker: str, df_full_1d: pd.DataFrame, tc: float,
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Entry":   st.column_config.NumberColumn(width=95,  format="%.6g"),
            "SL":      st.column_config.NumberColumn(width=95,  format="%.6g"),
            "TP (2R)": st.column_config.NumberColumn(width=100, format="%.6g"),
        },
    )
    ...
    for i, sig in enumerate(top_20):
        ...
        # Score bar (visual)
        score_pct = min(int(sig["score"]), 100)
        ...
        with st.expander(header, expanded=(i < 5)):
            ...
            with col_l:
                ...
                # The scanner always uses 0% retracement (immediate entry at candle
                # close) because it maximises trade capture rate across all coins.
                # Historical data shows 0% retrace has the highest fill rate; deeper
                # retracements filter out lower-quality setups but miss many winners.
                _bar_off  = sig.get("bar_offset", 1)
                if _bar_off == 1:
                    _entry_timing = (
                        "Candle just closed — <b style='color:#3fb950;'>enter at market open of next candle</b>. "
                        "This is the highest-probability entry: you capture the full momentum move before it accelerates."
                    )
                    _entry_method = "Immediate (0% retrace) — highest fill probability, recommended for fresh signals."
                else:
                    _retrace_note = (
                        f"Signal is <b>{_bar_off - 1} candle(s) old</b> — price may have moved. "
                        "Consider waiting for a retracement back toward the candle close before entering, "
                        "or skip if price is already >1R away from the original entry."
                    )
                    _entry_timing = _retrace_note
                    _entry_method = "Retracement preferred — signal is not fresh. Wait for pullback to improve R:R."
                ...
                st.markdown(
                    ...
                    f'<div><div style="color:#8892b0;font-size:11px;">ENTRY (at close)</div>'
                    f'<div style="color:#ccd6f6;font-weight:700;">{sig["entry"]:.6g}</div></div>'
                    f'<div><div style="color:#8892b0;font-size:11px;">STOP LOSS (1.5%)</div>'
                    ...
                    f'</div>'
                    f'<div style="margin-top:10px;padding-top:8px;border-top:1px solid #1e3a5f;">'
                    f'<div style="color:#8892b0;font-size:11px;text-transform:uppercase;'
                    f'letter-spacing:1px;margin-bottom:4px;">Entry Method</div>'
                    f'<div style="color:#ccd6f6;font-size:12px;line-height:1.5;">'
                    f'<b style="color:#58a6ff;">{_entry_method}</b><br>'
                    f'{_entry_timing}</div>'
                    f'</div>'
                    f'</div>',
                    unsafe_allow_html=True,
            ...
            with col_r:
                ...
                score_color = (
                    "#3fb950" if sig["score"] >= 70 else
                    "#e3b341" if sig["score"] >= 50 else
                    "#f85149"
                )
                st.markdown(
                    ...
                    f'{sig["score"]:.0f}<span style="font-size:16px;color:#8892b0;">/100</span></div>'
            ...
            if _bt_res or _ml_res:
                ...
                _bt_rows = (
                    (
                        ...
                        f'<span style="color:#8892b0;font-size:12px;">📊 Win Rate 2R (3%)</span>'
                        f'<span style="color:{"#3fb950" if _bt_res["win_2r"]>=55 else "#e3b341" if _bt_res["win_2r"]>=45 else "#f85149"};font-size:13px;font-weight:700;">'
                        f'{_bt_res["win_2r"]:.1f}% <span style="font-size:10px;color:#8892b0;">({_bt_res["n"]} setups)</span></span></div>'
                        f'<div style="display:flex;justify-content:space-between;padding:5px 0;">'
                        f'<span style="color:#8892b0;font-size:12px;">📊 Win Rate 3R (4.5%)</span>'
                        f'<span style="color:{"#3fb950" if _bt_res["win_3r"]>=50 else "#e3b341" if _bt_res["win_3r"]>=40 else "#f85149"};font-size:13px;font-weight:700;">'
                        f'{_bt_res["win_3r"]:.1f}%</span></div>'
                        f'<div style="display:flex;justify-content:space-between;padding:5px 0;">'
                        f'<span style="color:#8892b0;font-size:12px;">💰 Expected Value (2R)</span>'
                        f'<span style="color:{"#3fb950" if _bt_res["ev_2r"]>0 else "#f85149"};font-size:13px;font-weight:700;">'
                        f'{_bt_res["ev_2r"]:+.2f}R</span></div>'
                        f'<div style="display:flex;justify-content:space-between;padding:5px 0;">'
                        f'<span style="color:#8892b0;font-size:12px;">⏱ Avg Hold (bars)</span>'
                        f'<span style="color:#ccd6f6;font-size:13px;font-weight:700;">'
                        f'{_bt_res["avg_bars"]:.1f}</span></div>'
                    ) if _bt_valid else
                ...
                if _ai_res:
                    ...
                    _risk_row = (f'<div style="color:#8892b0;font-size:11px;border-top:1px solid #21262d;'
                                 f'padding-top:6px;margin-top:6px;">⚠️ Risk: {_ai_res["risk"]}</div>'
                    ...
                    _ai_block = (
                        ...
                        f'letter-spacing:1px;margin-bottom:6px;">🤖 AI Final Verdict{_c_badge}</div>'
                        f'<div style="color:{_v_color};font-size:20px;font-weight:900;margin-bottom:6px;">{_v}</div>'
                        f'<div style="color:#ccd6f6;font-size:12px;line-height:1.5;">{_ai_res.get("rationale","")}</div>'
                        f'{_risk_row}'
                        + (f'<div style="color:#3a3f4b;font-size:10px;margin-top:4px;">{_src}</div>' if _src and _src != "error" else "")
                        + f'</div>'
                    )

                _ml_color = "#3fb950" if _ml_res["pct"] >= 70 else "#e3b341" if _ml_res["pct"] >= 55 else "#f85149"
                _edge_bt  = (f' · {_bt_res["win_2r"]:.0f}% hist win-2R · EV {_bt_res["ev_2r"]:+.2f}R'
                             if _bt_valid else "")
                _html = (
                    ...
                    + _edge_bt
                    + f' · Score {sig["score"]:.0f}/100 · {sig["regime"]} regime</div></div>'
                    + _ai_block
                    + f'</div>'
                )
