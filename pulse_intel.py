...
def get_solana_flow_intel(symbol: str, api_key: str = "") -> dict:
    ...
    if base not in _SYMBOL_TO_SPL_MINT:
        return {
            "ok": False, "supported": False,
            "score": 0, "label": "N/A", "color": "#8892b0",
            "detail": f"{base} not in Solscan SPL-token map (add mint address to _SYMBOL_TO_SPL_MINT)",
...
def _fetch_lunarcrush_coin(symbol_upper: str, api_key: str) -> Optional[dict]:
    ...
    if cached is not None:
        return cached
    try:
        url = f"{_LUNARCRUSH_BASE}/public/coins/{symbol_upper}/v1"
        resp = requests.get(
            url,
            headers={
...
def get_social_intel(symbol: str, api_key: str = "") -> dict:
    ...
    if not api_key:
        return {
            ...
            "detail": (
                ...
                "lunarcrush.com/developers — paste it in the Pulse sidebar."
            ),
    ...
    if not data:
        return {
            "ok": False, "supported": True,
            "score": 0, "label": "API ERROR", "color": "#f85149",
            "detail": f"LunarCrush had no data for {base} (not tracked, rate-limited, or bad key)",
...
def _fetch_binance_fut_oi_hist(symbol_usdm: str) -> Optional[dict]:
    ...
    if cached is not None:
        return cached
    try:
        # period=1d, limit=2 returns the two most recent 24h buckets. The newer
        # one is "current" and the older one is "24h ago".
        resp = requests.get(
            f"{_BINANCE_FUT_BASE}/futures/data/openInterestHist",
            params={"symbol": symbol_usdm, "period": "1d", "limit": 2},
            timeout=8,
            headers={"User-Agent": "Mozilla/5.0 PulseIntel/1.0"},
        )
        if resp.status_code != 200:
...
# ─────────────────────────────────────────────────────────────────────────────
# Composite — Phase 3: blend TVL + Flow + Social + Macro backdrop
# ─────────────────────────────────────────────────────────────────────────────
def get_pulse_intel(symbol: str,
    ...
    Phase 4 (current): TVL + Etherscan Flow + Solscan Flow + LunarCrush Social
                       + Macro backdrop modifier + Binance Derivatives Pulse.
    ...
    Composite weighting (per-token, before macro modifier):
      When modules are available, the following weights are used:
        - Exchange Flow (ETH or SOL, whichever applies): 35%
        - TVL:                                           25%
        - Social:                                        20%
    ...
    macro       = get_macro_intel()
    derivatives = get_derivatives_intel(symbol)   # Phase 4 — no API key needed
    ...
    # Base weights: Flow 35%, TVL 25%, Social 20%, Derivatives 20%
    # (Phase 4 rebalance — derivatives added, others trimmed proportionally.)
    ...
    if raw_weights:
        ...
        # Scale from [-10,+10] sub-score range up to [-12,+12] so macro modifier
        # can push to the full ±15 range without clipping the per-token portion.
    ...
    if derivatives.get("ok") and derivatives.get("supported"):
        # Strip " DERIV" suffix for cleaner verdict string
    ...
    # Agreement/divergence detection (only across per-token signals — now 4)
    _pt_dirs = []
    if tvl.get("ok") and tvl.get("supported"):
    ...
    phase_label = (f"4 (TVL + {flow_chain} Flow + Social + Macro{_deriv_tag})"
                   if active_flow else
                   f"4 (limited modules available{_deriv_tag})")
    ...
    return {
        ...
        "smart_money":      None,               # reserved for future (The Graph / whale wallets)
