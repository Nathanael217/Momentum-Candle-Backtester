...
def _scanner_mini_wfo(sig: dict, bt_results: dict) -> dict:
    ...
    if is_n < 5:
        return {
            ...
            "note":         f"Only {is_n} IS trades — insufficient for statistical validation (need ≥5). Interpret backtest with caution.",
        }
...
def _scanner_train_ml(sig: dict, method_cfg: dict) -> dict:
    ...
    # ── Time-series CV (walk-forward) with sample-weighted fit ──────────────
    ...
    try:
        ...
        for tr_idx, te_idx in tss.split(X):
            ...
            try:
                ...
                # Some models/pipelines don't accept sample_weight keyword
    ...
    try:
        try:
            ...
            # CalibratedClassifierCV accepts sample_weight in newer sklearn.
            # Older versions don't — fall back to unweighted fit.
            model.fit(X, y)
            _weighted_fit = False
    except Exception as e:
        return _heuristic_fallback(
...
def render_pulse_tab():
    ...
    Phase 3 (planned): LunarCrush social, smart money proxy.
    """
    ...
    st.markdown(
        ...
        'that reads on-chain data for any coin and tells you what is powering the price '
        ...
        '<b style="color:#3fb950;">Phase 1 — TVL (LIVE):</b> DefiLlama TVL tracker — works '
        'for ~30 DeFi tokens (AAVE, UNI, LDO, GMX, etc.) and ~20 L1 chains (ETH, SOL, AVAX). '
        '<br>'
        '<b style="color:#3fb950;">Phase 2 — Exchange Flow (LIVE):</b> Etherscan-based net '
        'CEX flow tracker for ~20 ERC-20 tokens. Detects whale deposits (bearish) vs '
        'withdrawals (bullish) over the last 100-200 transfers. Needs free Etherscan API key. '
        '<br><b style="color:#8892b0;">Phase 3 (next):</b> LunarCrush social pulse, '
        'Solscan flows for SOL tokens, smart money proxy via The Graph.</div>',
        unsafe_allow_html=True,
    )
    ...
    # ── API Key input (Etherscan) ────────────────────────────────────────────
    # Phase 2 needs an Etherscan key for exchange flow data. Free tier is
    # plenty (5 calls/sec, 100k/day). We persist in session state so user
    # only enters once. TVL still works without it.
    with st.expander("🔑 API Keys (Etherscan needed for Phase 2 flow data)",
                     expanded=not st.session_state.get("pulse_etherscan_key")):
        st.markdown(
            ...
            '<b style="color:#58a6ff;">Get a free Etherscan API key:</b> '
            'Sign up at <b>etherscan.io/apis</b> (no credit card). '
            'Free tier = 5 calls/sec, 100k calls/day — more than enough.<br>'
            'TVL data works WITHOUT the key. Exchange flow needs it.'
            '</div>',
            unsafe_allow_html=True,
        )
        st.text_input(
            ...
            help="Free at etherscan.io/apis → My API Keys",
        )
        if st.session_state.get("pulse_etherscan_key"):
            st.caption("✅ Etherscan key set — Phase 2 exchange flow active")
        else:
            st.caption("⚠️ No key — exchange flow card will show 'NO KEY' badge")
    ...
    if analyze_clicked:
        with st.spinner(f"Fetching on-chain data for {symbol}..."):
            try:
                # Pass the Etherscan key from session state so exchange flow runs
                _es_key = st.session_state.get("pulse_etherscan_key", "") or ""
                result = _pulse.get_pulse_intel(symbol, etherscan_api_key=_es_key)
    ...
    # ── Phase 3 placeholders (Social, Smart Money — coming) ────────────────
    for emoji, name, desc in [
        ("📣", "Social Pulse",   "Mention volume, sentiment, galaxy score (LunarCrush)"),
        ("🐋", "Smart Money",    "Large whale swaps and accumulation patterns (The Graph)"),
    ]:
        st.markdown(
            ...
            f'<div style="display:flex;justify-content:space-between;align-items:center;">'
            f'<div style="color:#8892b0;font-size:11px;text-transform:uppercase;'
            f'letter-spacing:1px;font-weight:700;">{emoji} {name}</div>'
            f'<span style="color:#8892b0;border:1px solid #30363d;'
            f'padding:1px 8px;border-radius:4px;font-size:10px;">PHASE 3 — COMING SOON</span>'
            f'</div>'
            f'<div style="color:#8892b0;font-size:11px;margin-top:4px;">{desc}</div>'
            f'</div>',
            unsafe_allow_html=True,
        )
...
def main():
    ...
    # ── Tab structure: Scanner + Pulse ────────────────────────────────────────
    tab_scanner, tab_pulse = st.tabs([
