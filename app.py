...
def generate_probability_anchor(outcomes, target_buckets, regime_zone,
    ...
    pf_str   = f"{pf:.2f}" if pf < 9.9 else "∞ (no losses)"
...
# ─── WFO Adaptive Tier Helpers ────────────────────────────────────────────────

def _get_wfo_tier(n_setups: int, timeframe: str) -> dict:
    """Return WFO tier based on number of qualifying setups."""
    if n_setups < 15:
        return {"tier": "none", "label": "Insufficient setups for validation", "min_needed": 15}
    elif n_setups < 30:
        return {"tier": "preliminary", "label": "Preliminary Validation (single IS/OOS split)", "min_needed": 15}
    elif n_setups < 50:
        return {"tier": "wfo_2cycle", "label": "2-Cycle Rolling WFO", "min_needed": 30}
    else:
        return {"tier": "wfo_full", "label": "Full WFO (3+ cycles)", "min_needed": 50}


def _get_adaptive_wfo_config(n_bars: int, n_setups: int, timeframe: str) -> dict:
    """
    Return adaptive IS/OOS window config based on data availability.
    Overrides the fixed WFO windows for preliminary and 2-cycle tiers.
    Returns dict with: is_bars, oos_bars, step, max_cycles, tier.
    """
    tier_info = _get_wfo_tier(n_setups, timeframe)
    tier = tier_info["tier"]

    if tier == "none":
        return {"is_bars": 0, "oos_bars": 0, "step": 0, "max_cycles": 0, "tier": tier}

    if tier == "preliminary":
        is_bars = int(n_bars * 0.65)
        oos_bars = n_bars - is_bars
        # Check minimum IS window contains at least 10 setups
        estimated_setup_density = n_setups / n_bars if n_bars > 0 else 0
        is_setups_est = int(is_bars * estimated_setup_density)
        if is_setups_est < 10:
            return {"is_bars": 0, "oos_bars": 0, "step": 0, "max_cycles": 0,
                    "tier": "none", "downgraded": True}
        return {"is_bars": is_bars, "oos_bars": oos_bars, "step": oos_bars,
                "max_cycles": 1, "tier": tier}

    if tier == "wfo_2cycle":
        is_bars = int(n_bars * 0.40)
        oos_bars = int(n_bars * 0.20)
        step = oos_bars
        estimated_setup_density = n_setups / n_bars if n_bars > 0 else 0
        is_setups_est = int(is_bars * estimated_setup_density)
        if is_setups_est < 10:
            ...
            oos_bars = n_bars - is_bars
            return {"is_bars": is_bars, "oos_bars": oos_bars, "step": oos_bars,
                    "max_cycles": 1, "tier": "preliminary", "downgraded": True}
        return {"is_bars": is_bars, "oos_bars": oos_bars, "step": step,
                "max_cycles": 2, "tier": tier}

    # wfo_full — use existing _WFO_WINDOWS but allow up to 5 cycles
    _WFO_WINDOWS_FULL = {
        "1D": {"is_bars": 252, "oos_bars": 63},
        "4H": {"is_bars": 1512, "oos_bars": 378},
        "1H": {"is_bars": 5040, "oos_bars": 1260},
    }
    cfg = _WFO_WINDOWS_FULL.get(timeframe, _WFO_WINDOWS_FULL["1D"])
    is_bars = cfg["is_bars"]
    oos_bars = cfg["oos_bars"]
    # Check if we have enough bars; if not, scale down
    if is_bars + oos_bars > n_bars:
        is_bars = int(n_bars * 0.40)
        oos_bars = int(n_bars * 0.20)
    estimated_setup_density = n_setups / n_bars if n_bars > 0 else 0
    is_setups_est = int(is_bars * estimated_setup_density)
...
def run_wfo_pipeline(df, qualifying_func, adx_df, timeframe, direction,
    ...
    for cycle_i, start in enumerate(starts):
        ...
        _raw_oos_pf = float(m_oos.get("profit_factor", 0.0)) if m_oos else 0.0
        oos_pf      = min(_raw_oos_pf, 9.99)  # cap sentinel — no-loss OOS is not truly infinite
        oos_wr     = float(m_oos.get("win_rate",       0.0)) if m_oos else 0.0
        oos_trades = int(m_oos.get("total_trades",     0))   if m_oos else 0
        ratio      = min(oos_pf / best_is_pf, 2.0) if best_is_pf > 0 else 0.0  # cap at 200%
    ...
    # Exclude sentinel 9.99 values (no-loss cycles) from mean to avoid skew;
    # they still count as "profitable" for pct_profitable.
    real_pfs   = [p for p in oos_pfs if p < 9.0]
    avg_oos_pf = float(np.mean(real_pfs)) if real_pfs else float(np.mean(oos_pfs))
...
def calc_metrics(trades: List[Dict]) -> Dict:
    ...
    pf     = gp / gl if gl > 0 else (9.99 if gp > 0 else 0.0)  # 9.99 = no losses recorded
        ...
        if m and m["total_trades"] >= 5:
            results.append({
                ...
                "profit_factor": round(m["profit_factor"], 2)
                    if not np.isinf(m["profit_factor"]) else 9.99,
...
def _scanner_fetch_candles(symbol: str, interval: str,
                            limit: int = 100, deep: bool = False) -> pd.DataFrame:
    """
    Fetch klines for symbol/interval from Binance.
    deep=False  → fetch `limit` candles (fast scan phase, typically 120).
    deep=True   → fetch max historical bars for the timeframe:
                  1h → 500 bars (~3 weeks), 4h → 350 (~7 weeks), 1d → 1000 (~3 years).
    Returns cleaned DataFrame or empty DataFrame on failure.
    No caching — called inside thread workers.
    """
    if deep:
        _deep_limits = {"1h": 500, "4h": 350, "1d": 1000, "1w": 300}
        limit = _deep_limits.get(interval, 500)
    urls = [
...
def _scanner_ai_verdict(sig: dict, ml: dict = None, bt: dict = None, wfo: dict = None) -> dict:
    """
    Call Groq LLM to synthesize all evidence: signal features, ML probability,
    full multi-method backtest comparison, WFO validation, and the enhanced trade plan prices.
    Returns dict with: verdict, confidence, rationale, risk, execution, conflicts, source.
    """
    api_key = st.session_state.get("groq_api_key", "")
    if not api_key:
        return {
    ...
    prompt = f"""You are a SKEPTICAL trading analyst. Your PRIMARY job is to find reasons NOT to take this trade. Only output TRADE if you cannot find a meaningful reason against it after checking every layer below. Be decisive. Cite specific numbers. No markdown.
...
=== CONFLICT CHECK (you MUST evaluate each) ===
1. Body vs Volume: High body but is volume rank confirming? Vol={sig['vol_mult']:.2f}x — suspicious if body>80% but vol<2x
2. ADX vs DI gap: ADX={sig['adx']:.1f} but DI gap={abs(sig['di_plus']-sig['di_minus']):.1f} — strong trend needs both high
3. ML vs Backtest: Does ML direction agree with backtest EV sign? Flag if they disagree.
4. Signal age vs best zone: If signal is {sig.get('bar_offset',1)} candles old and best zone is Sniper, that zone may already be past.
5. Derivatives vs price action: If OI rising but taker sell-side dominant, flag as short buildup not momentum.
6. Funding vs direction: If going LONG but funding > 0.05%, longs are already crowded — flag.
7. WFO vs Backtest: If WFO FAIL/INSUFFICIENT but backtest looks good, flag as overfit risk.

=== DECISION RULES (apply strictly, in order) ===
- If backtest EV < 0 with n >= 8: output NO TRADE regardless of other factors
- If backtest WR < 40% with n >= 8: output NO TRADE
- If WFO verdict is FAIL: output WAIT minimum, note overfit risk
- If ML < 45%: lean WAIT or NO TRADE
- If 2+ conflicts detected above: lean WAIT minimum
- If signal age > 2 candles and best zone requires retrace: output WAIT
- If all engines agree AND WFO PASS AND no conflicts: HIGH confidence TRADE

Respond in EXACTLY this format, no extra text, no markdown:
VERDICT: [TRADE / WAIT / NO TRADE]
CONFIDENCE: [HIGH / MEDIUM / LOW]
CONFLICTS: [List any conflicts found, or "None detected" — be specific with numbers]
RATIONALE: [3 sentences max. Lead with the strongest factor for your verdict. Cite specific win rate, EV, ML%, WFO verdict, and any conflict.]
EXECUTION: [If TRADE: exact zone name, entry price, SL price, TP1 and TP2 prices, management style. If WAIT: what specific condition must change. If NO TRADE: what specifically disqualifies it.]
RISK: [1 sentence — name the SPECIFIC failure mode for this exact setup configuration, not a generic reversal warning.]"""
    ...
    try:
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": (
                            "You are a SKEPTICAL systematic momentum trading analyst. "
                            "Your job is to find reasons NOT to take the trade. "
        ...
        _model_used = st.session_state.get("groq_model", "llama-3.3-70b-versatile")
        return {
            "verdict":    verdict,
            "confidence": confidence,
            "rationale":  rationale,
...
def _scanner_quick_backtest(sig: dict) -> dict:
    ...
    # ── Deep fetch: maximum historical bars for this timeframe ────────────────
    # 1h→500 (~3 weeks), 4h→350 (~7 weeks), 1d→1000 (~3 years)
    df = _scanner_fetch_candles(symbol, interval, deep=True)

    # ── Data quality note for UI ──────────────────────────────────────────────
    _n_fetched = len(df)
    _warmup    = 30   # bars needed for ATR/EMA indicators to stabilize
    _backtest_bars = max(0, _n_fetched - _warmup)

    _ideal = {"1h": 500, "4h": 350, "1d": 1000}.get(interval, 500)
    if _n_fetched == 0:
        return {"error": "No data available", "n": 0}
    elif _n_fetched < 30:
        return {"error": f"Only {_n_fetched} bars available (need ≥ 30)", "n": 0}
    elif _n_fetched < _ideal * 0.4:
        _data_note = (f"⚠️ New coin — only {_n_fetched} bars available "
                      f"(ideal: {_ideal}). Results have lower confidence.")
        _data_note_type = "warning"
    elif _n_fetched < _ideal * 0.7:
        _data_note = (f"ℹ️ {_n_fetched} bars fetched of ideal {_ideal}. "
                      f"Adequate for {_backtest_bars} backtest bars.")
        _data_note_type = "info"
    else:
        _data_note = (f"✅ {_n_fetched} bars fetched — "
                      f"{_backtest_bars} effective backtest bars after indicator warmup.")
        _data_note_type = "ok"
        ...
        for sl_label, sl_pct_val in [("Fixed SL", FIXED_SL), ("ATR SL", atr_sl_pct)]:
            for mgmt in MGMT_MODES:
              for tp_mult in TP_MULTS:
                ...
                for i in range(14, n_df - 2):
                    ...
                    if result in ("WIN", "LOSS"):
                        trades_raw.append({
                            ...
                            "r_mult":    r_mult,
                            "bars_held": bars_held,
                            "bar_index": i,       # needed for time-decay weighting
                        })
                ...
                # Recent trades count more than old ones to reflect current market.
                # last 25% of bars → 1.0, 25–50% → 0.75, older half → 0.50
                _nd = n_df
                def _tdw(bar_idx):
                    age_pct = ((_nd - 1 - bar_idx) / max(_nd - 1, 1))
                    if   age_pct <= 0.25: return 1.00
                    elif age_pct <= 0.50: return 0.75
                    else:                 return 0.50

                weights   = [_tdw(t["bar_index"]) for t in trades_raw]
                total_w   = sum(weights)
                rs_raw    = [t["r_mult"] for t in trades_raw]

                # Weighted EV
                weighted_ev = sum(r * w for r, w in zip(rs_raw, weights)) / max(total_w, 1e-9)

                # Weighted win-rate (wins weighted by recency)
                win_w  = sum(w for r, w in zip(rs_raw, weights) if r > 0)
                wr_w   = win_w / max(total_w, 1e-9)

                # Raw (unweighted) for display context
                wins_raw = [r for r in rs_raw if r > 0]
                wr_raw   = len(wins_raw) / len(rs_raw)
                avg_b    = float(np.mean([t["bars_held"] for t in trades_raw]))

                method_results[key] = {
                    "zone": zone_name, "sl_label": sl_label, "mgmt": mgmt, "tp_mult": tp_mult,
                    "n": len(trades_raw),
                    "win_rate":     round(wr_w * 100, 1),    # time-decay weighted
                    "win_rate_raw": round(wr_raw * 100, 1),  # unweighted for reference
                    "ev":           round(weighted_ev, 3),   # time-decay weighted
                    "avg_r":        round(float(np.mean(rs_raw)), 3),
    ...
    return {
        ...
        "best_key":       best_key,
        "best":           best,
        # Data quality metadata shown in UI
        "data_note":      _data_note,
        "data_note_type": _data_note_type,
        "n_bars_fetched": _n_fetched,
        "n_backtest_bars": _backtest_bars,
    }
...
def _scanner_heuristic_ml(sig: dict) -> dict:
    """
    Fallback heuristic probability when real ML has not been trained.
    Uses hand-coded feature weights as a substitute. Clearly labelled in UI.
    """
    score = 0.0
    total = 0.0

    body_score  = min(sig["body_pct"] / 90.0, 1.0);          score += body_score  * 2.0; total += 2.0
    vol_score   = min(max(sig["vol_mult"] - 1.0, 0) / 4.0, 1.0); score += vol_score * 1.5; total += 1.5
    adx_score   = min(sig["adx"] / 40.0, 1.0);               score += adx_score   * 1.5; total += 1.5
    di_gap      = max((sig["di_plus"] - sig["di_minus"]) if sig["direction"]=="long"
                      else (sig["di_minus"] - sig["di_plus"]), 0)
    di_score    = min(di_gap / 30.0, 1.0);                    score += di_score    * 1.0; total += 1.0
    ema_score   = 1.0 if sig.get("ema_full") else (0.5 if sig.get("ema_partial") else 0.0)
    score += ema_score * 1.0; total += 1.0
    regime_sc   = sig.get("regime_score", 0) / 100.0;        score += regime_sc   * 2.0; total += 2.0
    score += sig.get("candle_rank", 0.5) * 0.5;              total += 0.5
    score += sig.get("vol_rank", 0.5) * 0.5;                 total += 0.5
    atr_sc      = min(max(sig.get("atr_ratio", 1.0) - 0.7, 0) / 1.3, 1.0)
    score += atr_sc * 0.5;                                    total += 0.5

    prob = max(0.30, min(0.95, score / max(total, 1e-9)))
    return {
...
# ── Feature names used by the inline ML trainer ───────────────────────────────
_ML_FEATURE_NAMES = [
    "body_pct", "vol_mult", "atr_ratio", "adx",
    "di_gap_signed", "ema_aligned", "candle_rank", "vol_rank", "regime_score_norm",
]


def _scanner_train_ml(sig: dict, bt_results: dict) -> dict:
    """
    Train a real ML model inline on historical qualifying candles for this coin.
    Auto-selects algorithm based on sample count:
      n < 50   → Logistic Regression  (simple, won't overfit small samples)
      50–150   → Random Forest        (handles non-linear patterns, gives feature importance)
      150+     → Gradient Boosting    (best generalization, industry standard for financial data)

    Uses TimeSeriesSplit cross-validation (never tests on training data).
    Returns dict with: probability, pct, label, method, n_samples, cv_accuracy,
                       model_name, feature_importance, confidence_note.
    """
    from sklearn.preprocessing import StandardScaler
    from sklearn.model_selection import TimeSeriesSplit
    from sklearn.metrics import accuracy_score
    ...
    # ── Fetch historical data (same deep fetch as backtest) ───────────────────
    df = _scanner_fetch_candles(symbol, interval, deep=True)
    if df.empty or len(df) < 40:
        return _scanner_heuristic_ml(sig)

    # ── Build training dataset ────────────────────────────────────────────────
    # Each qualifying historical candle → one training example.
    # Label: did price reach 2R TP before SL hit? (same logic as backtest)
    _etp      = sig.get("_trade_plan", {})
    atr_sl_pct = (_etp.get("sl_dist_pct", 2.0) or 2.0) / 100.0
    atr_sl_pct = max(0.008, min(0.06, atr_sl_pct))
    min_body  = abs(sig["body_pct"]) * 0.70 / 100
    ...
    # Use the best method from backtest to determine entry/SL parameters
    best     = bt_results.get("best", {}) if bt_results else {}
    zone_nm  = best.get("zone",     "Aggressive")
    sl_label = best.get("sl_label", "Fixed SL")
    tp_mult  = best.get("tp_mult",  2.0)
    ret_frac = {"Aggressive": 0.0, "Standard": 0.382, "Sniper": 0.618}.get(zone_nm, 0.0)
    expiry   = 0 if zone_nm == "Aggressive" else 3
    ...
    for i in range(14, n_df - MAX_HOLD - 2):
        bar      = df.iloc[i]
        body_pct = float(bar.get("body_pct", 0) or 0)
        vol_mult = float(bar.get("vol_mult",  0) or 0)
        is_bull  = body_pct > 0
        ...
        # Entry / SL / TP using the best method's config
        if direction == "long":
            entry_t = max(round(close_v - body_abs * ret_frac, 8), open_v * 1.001)
            if sl_label == "ATR SL":
                _sl_anch = float(bar.get("low", close_v)) - atr14 * 0.5
                _sl_anch = max(_sl_anch, close_v * 0.94)
                _sl_anch = min(_sl_anch, close_v * 0.992)
                if entry_t <= _sl_anch: continue
                _sp = max(0.008, min(0.06, (entry_t - _sl_anch) / entry_t))
                sl_px = round(entry_t - entry_t * _sp, 8)
            else:
                sl_px = round(entry_t * (1 - FIXED_SL), 8)
            risk  = entry_t - sl_px
            tp_px = entry_t + tp_mult * risk
        else:
            entry_t = min(round(close_v + body_abs * ret_frac, 8), open_v * 0.999)
            if sl_label == "ATR SL":
                _sl_anch = float(bar.get("high", close_v)) + atr14 * 0.5
                _sl_anch = min(_sl_anch, close_v * 1.06)
                _sl_anch = max(_sl_anch, close_v * 1.008)
                if entry_t >= _sl_anch: continue
                _sp = max(0.008, min(0.06, (_sl_anch - entry_t) / entry_t))
                sl_px = round(entry_t + entry_t * _sp, 8)
            else:
                sl_px = round(entry_t * (1 + FIXED_SL), 8)
            risk  = sl_px - entry_t
        ...
        # Simulate forward to label: 1=win (TP hit), 0=loss (SL hit)
        label     = None
        filled    = (ret_frac == 0.0)
        fill_bar  = i if filled else None
        cur_sl    = sl_px
        ...
        for j in range(i + 1, min(i + 1 + MAX_HOLD + max(expiry, 0) + 1, n_df)):
            ...
            if not filled:
                fill_ok = (lo <= entry_t if direction == "long" else hi >= entry_t)
                if fill_ok:
                    filled = True; fill_bar = j
                    scan_end = min(j + 1 + MAX_HOLD, n_df)
                else:
                    if expiry > 0 and (j - i) >= expiry: break
                    continue
            if j >= scan_end: break
            sl_hit = (lo <= cur_sl if direction == "long" else hi >= cur_sl)
            tp_hit = (hi >= tp_px if direction == "long" else lo <= tp_px)
            if sl_hit: label = 0; break
            if tp_hit: label = 1; break

        if label is None or not filled:
            continue
        ...
        if adx_df is not None and not adx_df.empty and i < len(adx_df):
            try:
                ...
                di_plus_v = float(adx_df["di_plus"].iloc[i])
                di_minus_v= float(adx_df["di_minus"].iloc[i])
            except Exception:
                pass

        ema5_v  = float(bar.get("ema5",  close_v) or close_v)
        ema15_v = float(bar.get("ema15", close_v) or close_v)
        ema21_v = float(bar.get("ema21", close_v) or close_v)
        if direction == "long":
            ema_aligned = 1 if (ema5_v > ema15_v > ema21_v) else 0
            di_gap_signed = di_plus_v - di_minus_v
        else:
            ema_aligned = 1 if (ema5_v < ema15_v < ema21_v) else 0
            di_gap_signed = di_minus_v - di_plus_v

        X_rows.append([
            abs(body_pct),                              # body_pct
            float(vol_mult),                            # vol_mult
            float(bar.get("atr_ratio", 1.0) or 1.0),  # atr_ratio
            adx_v,                                      # adx
            di_gap_signed,                              # di_gap_signed
            float(ema_aligned),                         # ema_aligned
            float(bar.get("candle_rank_20", 0.5) or 0.5), # candle_rank
            float(bar.get("vol_rank_20", 0.5) or 0.5),    # vol_rank
            float(bar.get("atr_ratio", 1.0) or 1.0) / 100.0,  # regime proxy (atr_ratio norm)
        ])
        y_labels.append(label)

    n = len(y_labels)
    if n < 20:
        # Not enough labeled examples — fall back to heuristic
        result = _scanner_heuristic_ml(sig)
        result["confidence_note"] = (
            f"Only {n} labeled examples from historical data "
            f"(need ≥ 20 for ML). Showing heuristic estimate."
        )
        return result
    ...
    for col in range(X.shape[1]):
        ...
        med      = np.median(finite) if len(finite) > 0 else 0.0
        X[:, col] = np.where(np.isfinite(col_vals), col_vals, med)

    # ── Auto-select model based on sample count ───────────────────────────────
    if n < 50:
        from sklearn.linear_model import LogisticRegression
        model      = LogisticRegression(C=0.5, solver="lbfgs", max_iter=1000, random_state=42)
        needs_scale= True
        model_name = f"Logistic Regression (n={n}, <50 samples)"
        level      = 1
    elif n < 150:
        ...
        needs_scale= False
        model_name = f"Random Forest (n={n}, 50–150 samples)"
        level      = 2
    else:
        ...
        needs_scale= False
        model_name = f"Gradient Boosting (n={n}, 150+ samples)"
        level      = 3

    # Scale if needed (LR only)
    if needs_scale:
        from sklearn.preprocessing import StandardScaler
        scaler = StandardScaler()
    ...
    n_splits = min(5, max(2, n // 15))
    tscv     = TimeSeriesSplit(n_splits=n_splits)
    cv_scores = []
    for tr_idx, va_idx in tscv.split(X_fit):
        if len(va_idx) < 3: continue
        try:
            model.fit(X_fit[tr_idx], y[tr_idx])
            preds = model.predict(X_fit[va_idx])
            cv_scores.append(accuracy_score(y[va_idx], preds))
        except Exception:
            pass
    cv_acc = float(np.mean(cv_scores)) if cv_scores else 0.5
    ...
    try:
        ...
        result = _scanner_heuristic_ml(sig)
        result["confidence_note"] = "ML training failed — showing heuristic."
        return result

    # ── Predict for current signal candle ────────────────────────────────────
    # Build feature vector for the current signal (same features, current values)
    sig_adx      = float(sig.get("adx", 20))
    sig_dip      = float(sig.get("di_plus", 0))
    sig_dim      = float(sig.get("di_minus", 0))
    sig_ema_al   = 1 if sig.get("ema_full") else (0 if not sig.get("ema_partial") else 0)
    sig_di_gap   = (sig_dip - sig_dim) if direction == "long" else (sig_dim - sig_dip)
    sig_body     = abs(float(sig.get("body_pct", 0.7)) or 0.7)
    sig_vol      = float(sig.get("vol_mult", 1.5) or 1.5)
    sig_atr_r    = float(sig.get("atr_ratio", 1.0) or 1.0)
    sig_crank    = float(sig.get("candle_rank", 0.5) or 0.5)
    sig_vrank    = float(sig.get("vol_rank", 0.5) or 0.5)

    x_sig = np.array([[sig_body, sig_vol, sig_atr_r, sig_adx, sig_di_gap,
                        sig_ema_al, sig_crank, sig_vrank, sig_atr_r / 100.0]])
    for col in range(x_sig.shape[1]):
        if not np.isfinite(x_sig[0, col]):
            x_sig[0, col] = float(np.nanmedian(X[:, col]))
    ...
    try:
        ...
        prob = 0.5
    prob = max(0.25, min(0.95, prob))

    # ── Feature importance ────────────────────────────────────────────────────
    fi = {}
    try:
        if hasattr(model, "feature_importances_"):
            fi = dict(zip(_ML_FEATURE_NAMES, model.feature_importances_.tolist()))
        elif hasattr(model, "coef_"):
            fi = dict(zip(_ML_FEATURE_NAMES,
                          [abs(c) for c in model.coef_[0].tolist()]))
    except Exception:
        pass

    # ── Confidence label based on CV accuracy ─────────────────────────────────
    if cv_acc >= 0.65:   conf = "HIGH"
    elif cv_acc >= 0.57: conf = "MEDIUM"
    else:                conf = "LOW"

    # ── Confidence note shown in UI ───────────────────────────────────────────
    n_pos  = int(y.sum()); n_neg = n - n_pos
    bal    = f"{n_pos} wins / {n_neg} losses ({n_pos/n*100:.0f}% win rate in training)"
    if cv_acc < 0.54:
        conf_note = (f"CV accuracy {cv_acc:.0%} is near random — "
                     f"no strong ML edge detected for this setup. Use with caution.")
    elif level == 1:
        conf_note = (f"Small sample ({n} examples) — Logistic Regression used. "
                     f"Gather more history for stronger predictions.")
    else:
        conf_note = (f"CV accuracy {cv_acc:.0%} across {n_splits}-fold walk-forward. "
                     f"Training: {bal}.")
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:12px;">
    ...
    <div style="margin-top:6px;">
      ...
      <div style="color:#ccd6f6;">Price already >1R from aggressive entry without a retrace</div>
            ...
            # ── Step 1: Backtest + ML + WFO ──────────────────────────────────
            _step1_col, _step2_col = st.columns([1, 1])
            with _step1_col:
                if st.button("📊 Step 1 — Backtest + ML + WFO",
                             key=f"step1_{_sym_key}_{i}",
                             use_container_width=True,
                             help="Deep-fetch historical bars, run backtest with time-decay weighting, train real ML model (auto-selects LR/RF/GBM), run WFO validation."):
                    with st.spinner("Fetching historical data…"):
                        _bt  = _scanner_quick_backtest(sig)
                    with st.spinner("Training ML model…"):
                        _ml  = _scanner_train_ml(sig, _bt)
                    with st.spinner("Running WFO validation…"):
            ...
            if _bt_res or _ml_res:
                ...
                _layer4 = f"ML Probability: {_ml_res_disp.get('pct', 50):.1f}% ({_ml_res_disp.get('label','—')})"
                _best_disp  = _bt_res.get("best", {})
                _bk_disp    = _bt_res.get("best_key", "—") or "—"
                _layer5 = (
                    f"Best Method: {_bk_disp} | "
                    f"WR={_best_disp.get('win_rate',0):.1f}% | "
                    f"EV={_best_disp.get('ev',0):+.2f}R | "
                    f"n={_best_disp.get('n',0)}"
                ) if _bk_disp != "—" else "Backtest: no valid method found"
                ...
                _intelligence_rows = [
                    ...
                    ("5. Backtest (500 bars)",    "#ccd6f6", _layer5),
                ...
                if _per_method:
                    ...
                    for _mk, _mv in sorted(_per_method.items(), key=lambda x: -x[1].get("ev", -99)):
                        ...
                        _mgmt_rows_html += (
                            f'<div style="{_row_bg}display:grid;grid-template-columns:3fr 1fr 1fr 1fr 1fr 1fr;'
                    ...
                    if _mgmt_rows_html:
                        _mgmt_table = (
                            ...
                            f'<div style="background:#161b22;display:grid;grid-template-columns:3fr 1fr 1fr 1fr 1fr 1fr;'
                ...
                if _ai_res:
                    ...
                    # Conflicts row
                    _conflicts_str = _ai_res.get("conflicts", "")
                    ...
                    if _conflicts_is_clean:
                        _conflicts_row = (
                            f'<div style="background:#0a1a0a;border-radius:4px;padding:6px 10px;margin-top:6px;">'
                            f'<span style="color:#3fb950;font-size:10px;text-transform:uppercase;'
                            f'letter-spacing:1px;">✅ Conflicts: </span>'
                            f'<span style="color:#ccd6f6;font-size:11px;">None detected</span>'
                            f'</div>'
                        )
                    elif _conflicts_str:
                        _conflicts_row = (
                            f'<div style="background:#1a1500;border-radius:4px;padding:6px 10px;margin-top:6px;">'
                            f'<span style="color:#e3b341;font-size:10px;text-transform:uppercase;'
                            f'letter-spacing:1px;">⚠️ Conflicts: </span>'
                            f'<span style="color:#ccd6f6;font-size:11px;">{_conflicts_str}</span>'
                            f'</div>'
                        )
                    else:
                        _conflicts_row = ""
                ...
                # ── ML model info row ─────────────────────────────────────────
                _ml_method    = _ml_res.get("method", "heuristic")
                _ml_model_nm  = _ml_res.get("model_name", "Heuristic")
                _ml_n         = _ml_res.get("n_samples", 0)
                _ml_cv        = _ml_res.get("cv_accuracy", None)
                _ml_conf_note = _ml_res.get("confidence_note", "")
                _ml_fi        = _ml_res.get("feature_importance", {})
                _ml_level     = _ml_res.get("level", 0)
                _ml_level_lbl = {1: "L1", 2: "L2", 3: "L3", 0: "H"}.get(_ml_level, "H")
                _ml_level_col = {1: "#8892b0", 2: "#58a6ff", 3: "#64ffda", 0: "#6e7681"}.get(_ml_level, "#6e7681")

                # Feature importance top-3
                _fi_html = ""
                if _ml_fi:
                    _top3 = sorted(_ml_fi.items(), key=lambda x: -x[1])[:3]
                    _fi_bars = "".join(
                        f'<div style="display:flex;align-items:center;gap:6px;padding:2px 0;">'
                        f'<div style="color:#8892b0;font-size:10px;width:95px;">{nm}</div>'
                        f'<div style="background:#21262d;border-radius:3px;height:6px;width:{min(int(imp*300),120)}px;background:{_ml_level_col};"></div>'
                        f'<div style="color:#ccd6f6;font-size:10px;">{imp:.3f}</div></div>'
                        for nm, imp in _top3
                    )
                    _fi_html = (
                        f'<div style="margin-top:4px;">'
                        f'<div style="color:#8892b0;font-size:10px;margin-bottom:3px;">Top predictors:</div>'
                        + _fi_bars + f'</div>'
                    )

                # Data quality note from backtest
                _bt_data_note      = _bt_res.get("data_note", "")
                _bt_data_note_type = _bt_res.get("data_note_type", "")
                _bt_note_col       = {"ok": "#3fb950", "info": "#58a6ff",
                                      "warning": "#e3b341", "": "#8892b0"}.get(_bt_data_note_type, "#8892b0")
                _bt_data_note_html = (
                    f'<div style="background:#0a0f1a;border-left:3px solid {_bt_note_col};'
                    f'padding:5px 10px;margin-top:4px;margin-bottom:4px;font-size:11px;">'
                    f'<span style="color:{_bt_note_col};">{_bt_data_note}</span></div>'
                ) if _bt_data_note else ""

                # ML confidence note
                _ml_conf_html = (
                    f'<div style="color:#8892b0;font-size:10px;margin-top:3px;">{_ml_conf_note}</div>'
                ) if _ml_conf_note else ""

                # CV accuracy badge
                _cv_badge = ""
                if _ml_cv is not None:
                    _cv_col = "#3fb950" if _ml_cv >= 0.65 else "#e3b341" if _ml_cv >= 0.55 else "#f85149"
                    _cv_badge = (
                        f'<span style="background:{_cv_col}22;border:1px solid {_cv_col};'
                        f'color:{_cv_col};font-size:9px;border-radius:3px;padding:1px 5px;margin-left:6px;">'
                        f'CV {_ml_cv:.0%}</span>'
                    )
                ...
                _html = (
                    ...
                    + _bt_data_note_html
                    + f'<div style="display:flex;justify-content:space-between;align-items:flex-start;padding:5px 0;">'
                    f'<div>'
                    f'<span style="color:#8892b0;font-size:12px;">🤖 ML Probability</span>'
                    f'<span style="background:{_ml_level_col}22;border:1px solid {_ml_level_col};color:{_ml_level_col};'
                    f'font-size:9px;border-radius:3px;padding:1px 5px;margin-left:5px;">{_ml_level_lbl}</span>'
                    f'<div style="color:#6e7681;font-size:10px;margin-top:2px;">{_ml_model_nm}</div>'
                    + _ml_conf_html
                    + _fi_html
                    + f'</div>'
                    ...
                    f'{_ml_res["pct"]:.1f}%'
                    + _cv_badge
                    + f'<span style="font-size:10px;color:#8892b0;margin-left:4px;">{_ml_res["label"]}</span></span></div>'
                    + _bt_rows
                    + _wfo_block_html
                    + _intel_expander_html
...
def render_wfo_tab(df, qualifying_func, ticker, timeframe, direction):
    ...
    if cycles:
        ...
        y_vals  = [min(c["oos_pf"], 5.0) for c in cycles]  # cap display at 5.0 — sentinel 9.99 would stretch chart
    ...
    with st.sidebar:
        ...
        with st.expander("🤖 AI Analysis (optional)", expanded=False):
            ...
            if "Groq" in _ai_provider:
                ...
                if _groq_key:
                    st.caption("✅ Groq key set — free AI analysis ready")
                    st.session_state["groq_model"] = st.selectbox(
                        ...
                        help="GPT OSS 120B = best quality. Llama 4 Scout = best for rapid scanning (5x higher TPM). Qwen 3 32B = best for conflict detection (reasoning model). Llama 3.3 70B = current default.",
                    )
                else:
...
★ Win Rate shown is from parameter sweep (no time exit applied). Open <b>Backtest Results</b> tab to see performance with your current time-exit settings.
    ...
    with t_backtest:
        ...
        # ── Walk-Forward Validation (Optional) — adaptive tier ────────────
        with st.expander("🔬 Walk-Forward Validation (Optional)", expanded=False):
            _n_setups_bt = len(qualifying) if not qualifying.empty else 0
            _wfo_tier_info = _get_wfo_tier(_n_setups_bt, _timeframe)
            _wfo_tier = _wfo_tier_info["tier"]

            st.markdown(
                f'<div style="background:#0d1117;border:1px solid #30363d;border-radius:6px;'
                f'padding:10px 14px;margin-bottom:10px;">'
                f'<span style="color:#8892b0;font-size:12px;">Qualifying setups: </span>'
                f'<span style="color:#ccd6f6;font-size:13px;font-weight:700;">{_n_setups_bt}</span>'
                f' &nbsp;→&nbsp; '
                f'<span style="color:#58a6ff;font-size:12px;">{_wfo_tier_info["label"]}</span>'
                f'</div>',
                unsafe_allow_html=True,
            )

            st.caption(
                "WFO tests if parameters found in-sample hold on unseen data. "
                "PASS = edge likely real. FAIL = likely overfit. "
                "Preliminary validation with < 30 setups should be treated as directional only."
            )

            if _wfo_tier == "none":
                st.info(
                    f"Minimum **15** qualifying setups needed for any validation. "
                    f"Currently: **{_n_setups_bt}**. "
                    f"Try increasing history length or relaxing filters."
                )
            elif _wfo_tier == "preliminary":
                st.info(
                    "This is a **single train/test split**, not full WFO. "
                    "With fewer than 30 setups, results are directional only."
                )
                if st.button("Run Preliminary Validation", key="wfo_prelim_btn",
                             type="primary", use_container_width=True):
                    _adap_cfg = _get_adaptive_wfo_config(len(df), _n_setups_bt, _timeframe)
                    with st.spinner("Running Preliminary Validation (single IS/OOS split)…"):
                        _prelim_result = run_wfo_pipeline(
                            df, detect_qualifying_candles, adx_df=None,
                            timeframe=_timeframe, direction=_direction,
                        )
                    st.session_state["wfo_bt_prelim"] = _prelim_result
                    st.rerun()

                _prelim = st.session_state.get("wfo_bt_prelim")
                if _prelim:
                    _p_summary = _prelim.get("summary", {})
                    _p_cycles = _prelim.get("cycles", [])
                    if _p_cycles:
                        _is_pf = _p_cycles[0].get("is_pf", 0)
                        _oos_pf = _p_cycles[0].get("oos_pf", 0)
                        _oos_wr = _p_cycles[0].get("oos_wr", 0)
                        if _oos_pf >= 1.2:
                            _pv_verdict, _pv_col = "PASS", "#3fb950"
                        elif _oos_pf >= 0.8:
                            _pv_verdict, _pv_col = "CAUTION", "#e3b341"
                        else:
                            _pv_verdict, _pv_col = "FAIL", "#f85149"
                        c1, c2, c3, c4 = st.columns(4)
                        c1.metric("IS Profit Factor", f"{_is_pf:.2f}")
                        c2.metric("OOS Profit Factor", f"{_oos_pf:.2f}")
                        c3.metric("OOS Win Rate", f"{_oos_wr:.0f}%")
                        c4.markdown(
                            f'<div style="text-align:center;padding-top:8px;">'
                            f'<span style="color:{_pv_col};font-size:24px;font-weight:900;">'
                            f'{_pv_verdict}</span></div>',
                            unsafe_allow_html=True,
                        )
                    else:
                        st.warning(_p_summary.get("reason", "Insufficient data for validation."))

            else:  # wfo_2cycle or wfo_full
                _adap_cfg = _get_adaptive_wfo_config(len(df), _n_setups_bt, _timeframe)
                _max_cyc = _adap_cfg["max_cycles"]
                _tier_label = _wfo_tier_info["label"]
                if _adap_cfg.get("downgraded"):
                    _tier_label += " (downgraded — IS window too small)"
                st.markdown(
                    f'<div style="color:#8892b0;font-size:12px;margin-bottom:8px;">'
                    f'Will run <b style="color:#ccd6f6;">{_max_cyc}</b> cycle(s) '
                    f'— {_tier_label}</div>',
                    unsafe_allow_html=True,
                )
                if st.button(f"Run {_wfo_tier_info['label']}", key="wfo_adaptive_btn",
                             type="primary", use_container_width=True):
                    with st.spinner(f"Running {_wfo_tier_info['label']} ({_max_cyc} cycles)…"):
                        _adap_result = run_wfo_pipeline(
                            df, detect_qualifying_candles, adx_df=None,
                            timeframe=_timeframe, direction=_direction,
                        )
                    st.session_state["wfo_bt_adaptive"] = _adap_result
                    st.rerun()

                _adap_res = st.session_state.get("wfo_bt_adaptive")
                if _adap_res:
                    _a_summary = _adap_res.get("summary", {})
                    _a_verdict = _a_summary.get("verdict", "FAIL")
                    _a_v_color = "#3fb950" if _a_verdict == "PASS" else "#f85149"
                    st.markdown(
                        f'<div style="background:#0d1117;border:2px solid {_a_v_color};'
                        f'border-radius:8px;padding:12px 16px;margin:8px 0;text-align:center;">'
                        f'<span style="color:{_a_v_color};font-size:24px;font-weight:800;">'
                        f'{_a_verdict}</span></div>',
                        unsafe_allow_html=True,
                    )
                    c1, c2, c3 = st.columns(3)
                    c1.metric("Avg OOS PF", f"{_a_summary.get('avg_oos_pf', 0):.2f}")
                    c2.metric("% Profitable", f"{_a_summary.get('pct_profitable_cycles', 0):.0f}%")
                    c3.metric("Cycles", str(_a_summary.get("n_cycles", 0)))
        ...
        if not best_trades:
            ...
            st.dataframe(
                display_log.style
                    ...
                    .map(color_exit,   subset=["Exit Type"]),
                use_container_width=True,
                height=520,
